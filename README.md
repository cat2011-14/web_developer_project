...
#### The .reduce() Method
.reduce() Метод возвращает одно значение после перебора элементов массива, тем самым уменьшая массив. 
```
...
```
Теперь давайте рассмотрим использование .reduce()из примера выше:
